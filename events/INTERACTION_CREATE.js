...
module.exports.run = async (client, interaction) => {
    ...
    if (!content) {
      ...
      async function getMember(id) {
        if (!id) return;
        const member = client.guilds.cache.get(interaction.guild_id).members.cache.get(id)
        return member;
      }
      ...
      interaction.member = await interaction.guild.members.cache.get(interaction.author.id)
      ...
      if (args) {
        ...
        interaction.getMember = async (id) => await getMember(id)

    client.slashcommands.forEach(command => {
      if (slashcmd === command.name) {
        if (!interaction.guild) return sendinvinsibleembed(`**You Must Use My Commands In a Guild!**`);
